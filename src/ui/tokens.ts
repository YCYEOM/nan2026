...
export const C = {
  ...
  // 우주복. **무채색이 아니라 `player[i]` 의 어둡고 탁한 파생이다** (DESIGN.md 원칙 7).
  // 흰 우주복은 어두운 배경에서 총알과 밝기로 경쟁하고, 이 게임은 색이 곧 판정이라
  // 그 경쟁에서 지는 쪽이 규칙이다. 같은 색상·다른 밝기면 소속만 말하고 안 싸운다.
  suit: {
    body: ["#123A47", "#4A3212"],    // 층 3 — 비발광. 몸통·다리
    panel: ["#1B5364", "#6B4A1C"],   // 층 3 — 헬멧 외피·백팩·가슴 띠
    trim: ["#00B4CC", "#CC8F00"],    // 층 2 — 좁게 발광. 어깨 띠·백팩 램프
  },
  /** 층 4 — 관절·부츠. 무채색은 전부 배경 톤으로 내린다. */
  joint: "#2A3159",
} as const;
